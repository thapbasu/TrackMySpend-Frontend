...
export default function ExpenseCategories() {
  ...
  // Get unique month/year combos from all expenses
  ...
  // Filter by selected month/year, then sort by category total
  const filteredByMonth = useMemo(() => {
    return groupedExpenses
      ...
      .sort((a, b) => {
        ...
        return sortOrder === "high" ? tb - ta : ta - tb;
      });
  }, [groupedExpenses, selectedMonth, selectedYear, sortOrder]);
  useEffect(() => {
    ...
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div className="flex gap-4 mt-6">
              <select
                ...
                className="px-4 py-2 rounded-xl border border-gray-200 bg-white/80 backdrop-blur-sm text-sm text-gray-700 font-medium shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent cursor-pointer hover:border-gray-300 transition-all"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {error && <ErrorState error={error} onRetry={fetchGroupedExpenses} />}

          {!loading && !error && groupedExpenses.length === 0 && <EmptyState />}

          {!loading && !error && filteredByMonth.length === 0 && searchTerm && (
          ...
          {!loading && !error && filteredByMonth.length > 0 && (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredByMonth.map((group) => {
                const categoryTotal = group.expenses.reduce(
